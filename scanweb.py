...
onlyfiles = next(os.walk(APP_FOLDER+"/html"))[2] #dir is your directory path as string

#text = 'citizen id  083-0174456 AA1254846 1-2001-01756-87-5'
df = read_csv(APP_FOLDER+'/html/'+onlyfiles[0]) 
columns = list(df)
pii_inventory = []
#d=[]
pii_categories =[]
...
for i in range(len(onlyfiles)):
    if ((onlyfiles[i][-4:]) == '.csv'):
        if (os.stat(APP_FOLDER +'/html/'+onlyfiles[i]).st_size) == 0:
            continue
        df = read_csv(APP_FOLDER +'/html/'+onlyfiles[i])
        for col in columns: 
...
report.to_csv(APP_FOLDER+'/report/webreport({0}).csv'.format(current_time))

os.remove(os.path.join(APP_FOLDER+"/html",APP_FOLDER+'/html/Mock_report(html).csv'))
